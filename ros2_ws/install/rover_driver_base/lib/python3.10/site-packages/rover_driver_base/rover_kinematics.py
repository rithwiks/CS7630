...
class RoverKinematics:
    ...
    def prepare_inversion_matrix(self,drive_cfg):
        ...
        temp = []
        for k in drive_cfg.keys():
            temp.append([1, 0, -drive_cfg[k].y])
            temp.append([0, 1, drive_cfg[k].x])
        W = np.array(temp)
        iW = np.linalg.pinv(W)
        return iW
    ...
    def prepare_displacement_matrix(self, motor_state_t1, motor_state_t2, drive_cfg):
        ...
        temp = []
        for k in drive_cfg.keys():
            disp = motor_state_t2.drive[k] - motor_state_t1.drive[k]
            corrected_disp = atan2(sin(disp), cos(disp))
            s = corrected_disp * drive_cfg[k].radius
            beta = (motor_state_t2.steering[k] + motor_state_t1.steering[k]) / 2.0
            temp.append(s*cos(beta))
            temp.append(s*sin(beta))
        S = np.array(temp)
        return S
